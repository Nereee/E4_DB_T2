...
CREATE TRIGGER eguneratu_estatistikak AFTER INSERT ON erreprodukzioak
FOR EACH ROW
BEGIN
    DECLARE audio_reproductions INT;
    
    SELECT COUNT(*) INTO audio_reproductions FROM erreprodukzioak WHERE id_audio = NEW.id_audio;
    
    UPDATE estatistikak
    SET erreprodukzio_total = audio_reproductions
    WHERE id_audio = NEW.id_audio;
END;
//
